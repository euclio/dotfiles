...
def _get_output(args):
    ...
    Arguments:
        args: A list representing the program and its arguments.
    """
    try:
        output = subprocess.check_output(args).decode('utf-8')
    except OSError as err:
        if err.errno == os.errno.ENOENT:
            logging.error("{} isn't installed.", args[0])
            return None            # The program isn't installed
        else:
            raise
...
def link_vim(vim_folder, backup_dir, dry_run):
    ...
    vim_version_output = _get_output(['vim', '--version'])
...
def link_git(filename, backup_dir, dry_run):
    """Special casing for removing unsupported git options."""
    git_version_output = _get_output(['git', '--version'])

    # Expected output:
    #   git version <VERSION>
    git_version = git_version_output.split()[-1]
    if git_version < '1.7.11':
        # push.default = simple is not supported.
        git_config = configparser.RawConfigParser()
        git_config.read(filename)
        try:
            push_default = git_config.get('push.default')
        except ValueError:
            return      # The option isn't set, don't worry about it

        if push_default == 'simple':
            git_config.remove_option('push.default')

        # Generate the new gitconfig
        generated_dir = os.path.join(os.path.basename(filename), 'gen')
        create_directory(generated_dir, dry_run)
        generated_gitconfig = os.path.join(generated_dir, '_gitconfig')
        with open(generated_gitconfig, 'wb') as new_git_config:
            new_git_config.write("""
# THIS FILE IS AUTOMATICALLY GENERATED. ANY CHANGES MADE WILL BE LOST.
#
""")
            new_git_config.write(git_config)

        link_file(generated_gitconfig, None, dry_run)
    else:
        link_file(filename, dry_run, backup_dir)
